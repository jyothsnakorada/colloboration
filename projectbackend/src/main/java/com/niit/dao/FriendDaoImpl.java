...
@Autowired
	...
	Session session=sessionFactory.getCurrentSession();
		SQLQuery sqlQuery=session.createSQLQuery("select * from user_s180250 where email in (select email from user_s180250 where email!=? minus (select fromId from friend_s180250 where toId=? union select toId from friend_s180250 where fromId=?))");
	...
	public List<User> listOfFriends(String email) {
		...
		Session session=sessionFactory.getCurrentSession();
		SQLQuery query=
		session.createSQLQuery("select * from user_s180250 where email in"
				+ "(select fromId from friend_s180250 where toId=? and status='A' "
				+ " union "
				+ "select toId from friend_s180250 where fromId=? and status='A')");
		query.setString(0, email);
